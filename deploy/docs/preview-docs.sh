...
#MOUNTS="${MOUNTS} -v ${DOCS_DIR}/config/_default/config.toml:/app/docs/config.toml:ro"
...
docker build -t bigshot-docs-previewer deploy/docs
docker run --rm -ti -p 1313:1313 ${MOUNTS} bigshot-docs-previewer $@

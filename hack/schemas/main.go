...
func main() {
	if err := generateSchemas(".", false, "schema", "schema"); err != nil {
		fmt.Println(err.Error())
	}
}

func generateSchemas(root string, dryRun bool, inputFile, outputFile string) error {
	input := filepath.Join(root, "pkg", "schema", inputFile+".go")
	output := filepath.Join(root, "docs", "content", "en", "schema", outputFile+".json")

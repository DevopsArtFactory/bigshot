...
const (
	...
	OS X:
		...
		$ bigshot completion bash > ~/.bigshot-completion  # for bash users
		$ bigshot completion zsh > ~/.bigshot-completion   # for zsh users
		$ source ~/.bigshot-completion
	Ubuntu:
		$ apt-get install bash-completion
		$ source /etc/bash-completion
		$ source <(bigshot completion bash) # for bash users
		$ source <(bigshot completion zsh)  # for zsh users

	Additionally, you may want to output the completion to a file and source in your .bashrc
`

	zshCompdef = "\ncompdef _bigshot bigshot\n"
)
